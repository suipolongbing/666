...
(function (pokerGame) {
    var CardTips = /** @class */ (function () {
        ...
        CardTips.prototype.recoverAllCards = function () {
            for (var i = 0; i < this.deckData.pokerLineList.length; i++) {
                var checkGroup = this.deckData.pokerLineList[i];
                console.log('pokerLineGroup', checkGroup);
            }
            for (var i = 0; i < this.deckData.pokerDeckList.length; i++) {
                var checkGroup = this.deckData.pokerDeckList[i];
                console.log('pokerDeckGroup', checkGroup);
            }
            var stc = this.deckData.StartCard;
            console.log('startGroup', stc);
            if (!GameMain.app.getValues) {
                return;
            }
            this.deckData.setKeepCardList(JSON.parse(GameMain.app.getValues));
            console.log('SaveStartCardList2', JSON.parse(GameMain.app.getValues));
            Laya.timer.once(1000, this, function () {
                GameMain.app.mWX.setUserValue('SaveStartCardList', null);
