...
class wxMinPro {
    ...
    public mCmd = {
        ...
        "SetUserValue": "1.0.1/jl/SetUserValue?", //设置期限道具
    };
    ...
    public mWxMenuButtonRect = null;
    // 返回微信右上角菜单，top和height，转换过的
    public getMenuButtonTop_CenterPoint(): Laya.Point {
        let rnt = new Laya.Point(0, 0);
        if (Laya.Browser.onMiniGame) {
            if (wxCore.uo.wxVersion() >= "2.1.0") {
                if (this.mWxMenuButtonRect == null && wx.getMenuButtonBoundingClientRect() != null) {
            ...
            if (this.mWxMenuButtonRect != null) {
                let h = this.mWxMenuButtonRect.height;
                let t = this.dealTopValue(this.mWxMenuButtonRect.top);
    ...
    private dealTopValue(value: number): number {
        if (value > 0) {
            return value;
        }
        else {
