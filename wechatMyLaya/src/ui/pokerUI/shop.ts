...
module pokerUI {
	...
	export class shop {
		...
		private shopConfigData: gameconfig.shopConfigData = new gameconfig.shopConfigData();
		public shopUI: ui.poker.ShopUI;
		private IsInit: boolean = false;

		public static ItemSelectedInPage1: gameconfig.shopItem;
		public static ItemSelectedInPage2: gameconfig.shopItem;
		public static ItemSelectedInPage3: gameconfig.shopItem;

		constructor() {
			Laya.loader.load([{ url: "res/atlas/UI.atlas", type: Laya.Loader.ATLAS }], Laya.Handler.create(this, this.onUILoad));
		}

		private IsFromGame: boolean = false;
		public OpenShop(isFromGame: boolean) {
		...
		public FlushData(userdata: pokerGame.UserData) {
		...
		private FlushSelectedItem(pageNum: number) {
			...
			switch (pageNum) {
				...
				case 3:
					...
					console.log('l11', this.shopConfigData.GetItem(IDStr, 3), IDStr);
					break;
			}
		...
		private SetCellDataPage1(cell: Laya.Box, index: number) {
		...
		private SetCellDataPage2(cell: Laya.Box, index: number) {
		...
		private SetCellDataPage3(cell: Laya.Box, index: number) {
		...
		private FlushPageData(page: number) {
			var dataArray = this.shopConfigData.getDataArray(page);
			var list = this.getPageList(page);
			...
			for (var i = 0; i < dataArray.length; i++) {
				...
				var cell: any = null;
				var data: gameconfig.shopItem = dataArray[i];
				for (var j = 0; j < cells.length; j++) {
					var cellTest = cells[j];
					if (data.ItemName == (<Laya.Label>cellTest.getChildByName("name")).text) {
						cell = cellTest;
						break;
					}
				}
		...
		private getPageList(page: number): Laya.List {
			var tRet: Laya.List = null;
			switch (page) {
				case 1:
		...
		private LoadAndChangeIcon(icon: Laya.Image, atlas: string, skin: string) {
			if (icon == null) {
				console.debug("Erro");
			}
		...
		private ChangeIconSkin(icon: Laya.Image, skin: string) {
			icon.skin = skin;

		}

		private setIconData(cell: any, itemData: gameconfig.shopItem) {
			var box = <Laya.Box>cell;
			switch (itemData.page) {
				case 1://背景
					{
						var icon = <Laya.Image>box.getChildByName("icon1");
				...
				case 2://扑克
					{
						...
						var icon3 = <Laya.Image>box.getChildByName("icon3");
		...
		private setCellData(cell: any, itemData: gameconfig.shopItem) {

			var box = <Laya.Box>cell;
			//var icon = <Laya.Image>box.getChildByName("icon");
			var name = <Laya.Label>box.getChildByName("name");
			var buy = <Laya.Button>box.getChildByName("buy");
			var trybtn = <Laya.Button>box.getChildByName("try");
			var usebtn = <Laya.Button>box.getChildByName("use");
			var usingText = <Laya.Text>box.getChildByName("using");
			...
			if (IsUsing) {
				...
				var IsPurchasedItemID: boolean = this.userdata.IsPurchasedItemID(itemData.ItemID);
		...
		public SetGoldText(goldNumText: string) {
			if (this.shopUI != null) {
